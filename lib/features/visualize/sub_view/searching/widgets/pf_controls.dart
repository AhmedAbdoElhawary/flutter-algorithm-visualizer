...
class SearchingAlgorithmControls extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final backwardValidation = ref.watch(instance.select((s) => s.hasSteps && !s.isAtStart));
    final forwardValidation = ref.watch(instance.select((s) => s.hasSteps && !s.isAtEnd));
    ...
    return AlgorithmControls(
      ...
      endOptionButtons: [
        CtrlButton(
          ...
          size: 18,
          onTap: notifier.clearWalls,
          messageTip: StringsManager.clearWalls,
        ),
        CtrlButton(
          icon: Icons.shuffle_rounded,
          size: 18,
          onTap: notifier.randomizeWalls,
          messageTip: StringsManager.randomWalls,
        ),
      ],
    );
  }
